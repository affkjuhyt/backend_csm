...
class BayesianPersonalizationRanking(object):
    ...
    def initialize_factors(self, train_data, k=25):
        ...
        self.item_factors = np.random.random_sample((len(self.book_ids), k))
        self.user_books = train_data.groupby('user_id')['book_id'].apply(lambda x: x.tolist()).to_dict()
    ...
    def draw(self, no):
        ...
        for _ in range(no):
            ...
            user_items = self.user_books[u]

            pos = random.choice(user_items)

            neg = pos
            while neg in user_items:
                neg = random.choice(self.book_ids)
...
def load_all_ratings(min_ratings=1):
    ...
    item_count = ratings[['book_id', 'rating']].groupby('book_id').count()
...
if __name__ == '__main__':
    ...
    bpr = BayesianPersonalizationRanking(save_path='./models/bpr/{}/'.format(datetime.now().strftime("%d")))
