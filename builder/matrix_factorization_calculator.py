...
class MatrixFactorization(object):
    ...
    def initialize_factors(self, ratings, k=25):
        ...
        self.book_ids = set(ratings['book_id'].values)
        self.item_counts = ratings[['book_id', 'rating']].groupby('book_id').count()
        self.item_counts = self.item_counts.reset_index()

        self.item_sum = ratings[['book_id', 'rating']].groupby('book_id').sum()
        self.item_sum = self.item_counts.reset_index()

        self.u_inx = {r: i for i, r in enumerate(self.user_ids)}
        self.i_inx = {r: i for i, r in enumerate(self.book_ids)}

        self.item_factors = np.full((len(self.i_inx), k), Decimal(0.1))
        self.user_factors = np.full((len(self.u_inx), k), Decimal(0.1))

        self.all_books_mean = calculate_all_books_mean(ratings)
    ...
    def predict(self, user, item):
        ...
        b_ui = self.all_books_mean + self.user_bias[user] + self.item_bias[item]
        prediction = b_ui + pq
    ...
    def train(self, ratings_df, k=40):
        ...
        ratings = ratings_df[['user_id', 'book_id', 'rating']].values
...
def calculate_all_books_mean(ratings):
    avg = ratings['rating'].sum() / ratings.shape[0]
    return Decimal(avg)


def load_all_ratings(min_ratings=1):
    columns = ['user_id', 'book_id', 'rating', 'type', 'rating_timestamp']
...
if __name__ == '__main__':
    ...
    MF = MatrixFactorization(save_path='./models/funkSVD/{}/'.format(datetime.now().strftime("%d")), max_iterations=40)
