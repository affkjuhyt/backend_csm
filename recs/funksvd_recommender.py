...
class FunkSVDRecs(base_recommender):
    ...
    def __init__(self, save_path='./builder/models/funkSVD/19/model/'):
    ...
    def recommend_items_by_ratings(self, user_id, active_user_items, num=6):
        ...
        rated_books = {book['book_id']: book['rating'] for book in active_user_items}
        recs = {}
        if str(user_id) in self.user_factors.columns:
            ...
            result = sorted_scores[:num + len(rated_books)].astype(decimal.Decimal)
            user_bias = 0
            ...
            recs = {r[0]: {'prediction': r[1] + float(self.item_bias[r[0]])}
                    for r in zip(result.index, result) if r[0] not in rated_books}
