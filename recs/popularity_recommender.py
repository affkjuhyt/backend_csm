...
class PopularityBasedRecs(base_recommender):
    ...
    def predict_score(self, user_id, item_id):
        avg_rating = Rating.objects.filter(~Q(user_id=user_id) &
                                           Q(book_id=item_id)).values('book_id').aggregate(Avg('rating'))
    ...
    def recommend_items(self, user_id, num=6):
        pop_items = Rating.objects.filter(~Q(user_id=user_id)).values('book_id').annotate(Count('user_id'),
                                                                                           Avg('rating'))
    ...
    def recommend_items_by_ratings(user_id, active_user_items, num=6):
        ...
        pop_items = Rating.objects.filter(~Q(book_id__in=item_ids)).values('book_id').annotate(Count('user_id'),
                                                                                           Avg('rating'))
        recs = {i['book_id']: {'prediction': i['rating__avg'], 'pop': i['user_id__count']} for i in pop_items}
        sorted_items = sorted(recs.items(), key=lambda item: -float(item[1]['pop']))[:num]
        return sorted_items

    @staticmethod
    def predict_score_by_ratings(item_id, books):
        item = Rating.objects.filter(book_id=item_id).values('book_id').annotate(Avg('rating')).first()
        if not item:
            return 0
